...
def process_inputs():
    ...
    firstQuart = float(sfirstQuart)
    secondQuart = float(ssecondQuart)
    targetFinal = float(stargetFinal)
    if firstQuart > 100.0 or firstQuart < 0 or secondQuart > 100.0 or secondQuart < 0:
        flash("Grades must be between 0-100!")
        return render_template("main_page.html", output="")

    # Do the stuff
    finalGrade = int(round(calc_target_final_grade(firstQuart, secondQuart, targetFinal)))
    if finalGrade <= 0:
        outString = "You could skip the final :)"
    elif finalGrade > 100:
        outString = "Oof! Choose a more realistic target grade :("
    else:
        outString = "Aim for a " + str(finalGrade) + " on the final for a final grade of " + str(int(targetFinal)) + " in the class."
    return render_template("main_page.html", output=outString)

def calc_target_final_grade(first, second, final):
    return (10 * (final - 0.5)) - (4.5 * (first + second))
